...
campoMultiplo.instanciar = function(seletor_campo, escopo){
	...
	busca.each(function(){
		...
		var quantidade_maxima, callback_onadd;
		...
		// Obtendo evento "onadd", chamado após um campo ser adicionado
		if($campo.is('[data-onadd]') && $.trim( $campo.attr('data-onadd') != '' )){
			callback_onadd = $.trim( $campo.attr('data-onadd') );
		} else {
			callback_onadd = undefined;
		}
		...
		if(total_campos_inserir > 0){
			for(var i in valores_campos){
				...
				if(i == 0){
					campoMultiplo.adicionar(id_campo, true, valores_campo, callback_onadd);
				} else {
					campoMultiplo.adicionar(id_campo, false, valores_campo, callback_onadd);
				}
			}
		} else {
			campoMultiplo.adicionar(id_campo, true, undefined, callback_onadd);
		...
		$botaoAdicionar.click(function(){
			...
			if((quantidade_maxima == 0) || (total_campos <= quantidade_maxima)){
				campoMultiplo.adicionar(id_campo, undefined, undefined, callback_onadd);
			} else {
...
campoMultiplo.adicionar = function(id_campo, primeiroCampo, valores_campo, callback){
	...
	if(primeiroCampo){
		...
		$trClonado.children('td.acoes').children('button').each(function(){
			...
			if($botaoAcao.hasClass('adicionar')){
				...
				$botaoAcao.css('display', 'inline-block');
	...
	// Executando callback após a adição de um novo campo.
	// O parâmetro 'this' do callback é sobrescrito pelo elemento
	// de destino, após a adição mais recente.
	if(callback){
		var callback_texto = callback.replace('this', 'td');
		callback = function(td){
			eval(callback_texto);
		};
		callback($elementoDestino);
	}
}
	...
	for(var nome_campo in valores_campo){
		...
		if($campo.is('select')){
			...
			$radios.each(function(){
				var $radio = $(this);
				if(valor.toString() == $radio.val()){
					$radio.prop('checked', true);
				}
			});
		} else if($campo.is("[type='file'].fileuploader")){
			// Campos de upload de arquivos, instanciados pela biblioteca "fileuploader.js"
			if(valor != ''){
				var id = $campo.attr('id');
				$campo.attr('data-value', valor);
				if(!fileUploader.upl.campos[id].multiplos_arquivos) $campo.attr("disabled", "disabled");
				fileUploader.upl.campos[id].iniciado_com_valor = true;
...
campoMultiplo.verificaValoresDuplicados = function(campoElementoDestino){
	...
	$elementoDestino.find('input:not([hidden]), select, textarea').each(function(i){
		...
		if($campo.is("[type='checkbox']")){
			...
			var valor_radio;
			if($radio.length > 0){
				valor_radio = ( $radio.val() ).toString();
			} else {
				valor_radio = '';
			}
			valores_atual += valor_radio;
		} else {
			valores_atual += ( $campo.val() ).toString();
		}
	});
	...
	if(checkExisteValoresDuplicados){
		...
		if($campoElementoDestino.is("[type='checkbox'], [type='radio']")){
