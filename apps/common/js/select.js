...
select.instanciar = function(seletor_campo, escopo){
	...
	busca.each(function(){
		...
		$select.on({
			// Open: Chamado após o campo ser aberto
			'select2:open': function(e, a, b, c){
				...
				var id_container = id_campo + '-container';
				$dropdown.attr('id', id_container);
				if(checkDentroCardVerde){
					$dropdown.addClass('dentro-card-verde');
				}
			},
...
select.obterMetadadosOpcaoSelecionada = function (select) {
	var $select = $(select);

	var metadados = $select.select2('data');
	if ($select.is('[multiple]')) {
		return metadados;
	} else {
		return metadados[0];
